...
export class AuthService {
  ...
  register(formData: FormData) {
    return this.http.post<any>(
      `${settings.currentApiUrl}/users/register`,
      formData
    );
    // .pipe(map(user => {
    //     if (user && user.token) {
  ...
  updateUser(formData: FormData) {
    return this.http.put<any>(
      `${settings.currentApiUrl}/users/update`,
      formData
    );
  }
  ...
  login(phoneNumber: any, password: any) {
    return this.http
      .post<any>(`${settings.currentApiUrl}/users/authenticate`, {
        phoneNumber,
        password,
      })
      .pipe(
    ...
    if (rMethod) {
      switch (rMethod) {
        case "momo":
          ...
          if (
            this.currentUser.momoDetails &&
            !this.currentUser.momoDetails.number
          )
        ...
        case "bank":
          ...
          //Return true if all bank details are present
          if (
            this.currentUser.bankDetails.accountNumber &&
            this.currentUser.bankDetails.swiftCode &&
            this.currentUser.bankDetails.bankName
          ) {
            return true;
          } //else show the show the error message
          else if (showConfirmBox) {
        ...
        default:
          if (showConfirmBox) {
            this.ChangeDetailsConfirm(
              "Please choose the correct prefered money reception method",
              data
            );
            return false;
          }
      ...
      if (showConfirmBox)
        this.ChangeDetailsConfirm(
          "Give us details of how you want us to pay you when you win.",
          data
        );
      return false;
    }
  ...
  ChangeDetailsConfirm(error: string, data: string) {
    if (confirm(error)) {
      this.router.navigate(["manage", "profile"], {
        queryParams: { returnUrl: this.router.url, urldata: data },
      });
    }
  }
  ...
  getUserRole(userId: string) {
    this.http
      .get(`${settings.currentApiUrl}/users/exists/${userId}`)
