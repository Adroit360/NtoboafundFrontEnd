...
export class AdbusinessesComponent implements OnInit {
  ...
  constructor(public businessService: BusinessService, public winnerSelectionService: WinnerSelectionService, private paymentService: PaymentService,
    ...
    this.paymentForm = new FormGroup({
      'amount': new FormControl(this.selectedBusinessPaymentDetails.amount, [Validators.required]),
      'transactionId': new FormControl(this.selectedBusinessPaymentDetails.transactionId, [Validators.required])
    })
  }
  ...
  getUnpaidWinnersCount() {
    this.businessService.getUnpaidWinnersCount().subscribe(
  ...
  changeSelectedBusiness(businessId: number) {
    ...
    if (this.selectedbusiness.status == "won" || this.selectedbusiness.status == "completed")
  ...
  addPaymentRecord() {
    ...
    this.paymentService.addPaymentRecord('bus', amount, transactionId, this.selectedbusiness.transferId).subscribe(
      (response: any) => {
  ...
  fixWinner(winnerId: number) {
    this.signalRService.fixWinner('business', this.selectedFixerPeriod, winnerId);

    this.winnerSelectionService.setFixedWinner(winnerId, this.participantsToFix);
  }

  unFixWinner(winnerId: number) {
    this.signalRService.unfixWinner('business', this.selectedFixerPeriod, winnerId);
 ...
 */
  ...
  showUnpaidWinners(shouldResetSelectedLkm = true) {
    ...
    this.filterText = "won";
    this.businessService.getBusinessesByType(this.usersTypeFilterText).then((value) => {
      this.recentBusinessStakes = [...this.businessService.allBusinesses];
  ...
  getPaymentRecord() {
    if (this.selectedbusiness.status == "completed") {
      this.paymentService.getPaymentByDetails("bus", this.selectedbusiness.id).subscribe(
        response => {
          console.log(response);
          if (response) {
            this.selectedBusinessPaymentDetails = response;
          }
          this.selectedBusinessPaymentDetails.amount = this.selectedbusiness.amountToWin;

          this.paymentForm.get('amount').setValue(this.selectedBusinessPaymentDetails.amount);
          this.paymentForm.get('transactionId').setValue(this.selectedBusinessPaymentDetails.transactionId);
        }
      )
    } else {
