...
export class ScholarshipComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.winnerSelectionService.isQuaterlyDrawOngoing.subscribe(
      (isOngoing: boolean) => {
  ...
  ngOnInit() {
    this.countDownService.QuaterlyDaysTime.subscribe((days: number) => {
      this.scholarshipDays = days;
    });
    this.countDownService.QuaterlyHoursTime.subscribe((hours: number) => {
      this.scholarshipHours = hours;
    });
    this.countDownService.QuaterlyMinutesTime.subscribe((minutes: number) => {
      this.scholarshipMinutes = minutes;
    });
    this.countDownService.QuaterlySecondsTime.subscribe((seconds: number) => {
      this.scholarshipSeconds = seconds;
    });
  ...
  paymentInitialized(_amount?) {
    ...
    if (this.authService.isAuthenticated) {
      ...
      if (
        ...
        this.http
          .post<any>(`${settings.currentApiUrl}/scholarships/addnew`, {
            amount,
            institution,
            program,
            studentId,
  ...
  scholarshipPaymentCallback(event) {
    ...
    if (event.success) {
      ...
      this.http
        .post<any>(
          `${settings.currentApiUrl}/transaction/verifyScholarshipPayment/${event.tx.txRef}`,
          {}
        )
        .subscribe(
          (response) => {
  ...
  getUserScholarships() {
    this.http
      .get(
        `${settings.currentApiUrl}/scholarships/foruser/${this.authService.currentUser.id}`
      )
      .subscribe(
  ...
  setRaveOptions(amount) {
    var isValid =
      ...
      this.authService.hasPaymentDetails(
        `${this.scholarshipForm.get("Institution").value}/*/${
          this.scholarshipForm.get("Program").value
        }/*/${this.scholarshipForm.get("StudentId").value}/*/${
          this.scholarshipForm.get("PlayerType").value
        }`
      );
  ...
  setPotentialReturns() {
    this.error = null;
    this.potentialReturns = ` GH₵ ${(
      this.selectedAmount * settings.scholarshipStakeOdds
    ).toLocaleString()}`;
  }
}
